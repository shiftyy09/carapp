...
class _MaintenanceDataInputScreenState extends State<MaintenanceDataInputScreen> {
  ...
  Future<void> _saveData() async {
    ...
    if (_beltDate != null) {
      ...
      print('⚙️ Saved beltChangeDate: ${prefs.getString('${key}beltChangeDate')}');
    }
    if (_brakeDate != null) {
      await prefs.setString('${key}brakeFluidDate', _brakeDate!.toIso8601String());
      print('⚙️ Saved brakeFluidDate: ${prefs.getString('${key}brakeFluidDate')}');
    }
          ...
          child: ListView(
            children: [
              ...
              ListTile(
                ...
                subtitle: Text(
                  _oilDate?.toLocal().toString().split(' ')[0] ?? 'Nincs kiválasztva',
              ...
              ListTile(
                ...
                subtitle: Text(
                  _beltDate?.toLocal().toString().split(' ')[0] ?? 'Nincs kiválasztva',
                  style: const TextStyle(color: Colors.orange),
                ),
                trailing: IconButton(
                  icon: const Icon(Icons.calendar_today, color: Colors.orange),
              ...
              ListTile(
                ...
                subtitle: Text(
                  _brakeDate?.toLocal().toString().split(' ')[0] ?? 'Nincs kiválasztva',
                  style: const TextStyle(color: Colors.orange),
                ),
                trailing: IconButton(
                  icon: const Icon(Icons.calendar_today, color: Colors.orange),
              ...
              ElevatedButton(
                style: ElevatedButton.styleFrom(backgroundColor: accentOrange, foregroundColor: Colors.black),
                onPressed: _saveData,
